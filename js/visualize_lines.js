var globeRadius = 100; // 值越大，曲线的高度越高
...
function makeConnectionLineGeometry( facility, landing, apogee ){
	...
	var midHeight = globeRadius * apogee / 6378.137; // 赤道半径6378.137km
	var midLength = globeRadius + midHeight;
	// var midHeight = globeRadius * apogee / 6378.137 * 0.6666;
	// var midLength = globeRadius + midHeight;

	// 交换方向，使得现在蜜罐出发，变成打向蜜罐
	//	start of the line
	var end = facility.center;

	//	end of the line
	var start = landing.center;
...
//	var endAnchor = start.clone().lerp(end,0.8333).normalize().multiplyScalar(anchorLength);
	...
	var vertexCountDesired = Math.floor( (distance + midHeight) * 0.1 + 1 );
