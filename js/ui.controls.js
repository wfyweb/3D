...
function showMiniMap(honeypot) {
		...
		if (swiperData.res.length) {
			swiperData.res.forEach(function (i, j) {
				if (missleName[i.hp_name] === hp_name) {
					wrapCon.empty()
					i.geoip.forEach(function (m, n) {
						var cityName = m.real_region_name || ''
						wrapCon.append('<div class="swiper-slide swiper-no-swiping" num='+ n +'>\
							<h1>'+m.ip+'</h1><p>'+m.country_name+'</p><p>'+ cityName +'</p>\
							</div>')
					})
					return false;
				}
...
function changeLang () {
	/*
		首先定义字典 atnData
		获取中英文 locale
		获取domArr，更具lang改变text
	*/
	var locale = localStorage.locale || 'en';
	var atn = $('.attack-type-name');
	var atnTitle = $('.attack-type-title')[0]
	var len = Object.getOwnPropertyNames(atnData).length
	for(var item = len; item--;) {
		var res = Object.getOwnPropertyNames(atnData)[item];
		if (locale === 'en') {
			atnTitle.innerHTML = atnTitleData.en
			atn[item].innerHTML = atnData[res].en
		} else {
			atnTitle.innerHTML = atnTitleData.cn
			atn[item].innerHTML = atnData[res].cn
		}
	}
}
...
var d3Graphs = {
	...
	windowResize: function() {
		...
		if (mediaType() === 'pc') {
			...
			$("#missileTypeBtns").css('left', 20); // 使右侧legend移动到左侧 w - missileButtonWidth - 20
			// $("#missileTypeBtns").css('top', h - missileButtonHeight - 15);
			$("#missileTypeBtns").css('top', '5.5rem');
			...
			if (swiperData.res.length) {
				swiperData.res.forEach(function (i, j) {
					if (missleName[i.hp_name] === hp_name) {
						...
						i.geoip.forEach(function (m, n) {
							var cityName = m.real_region_name || ''
							wrapCon.append('<div class="swiper-slide swiper-no-swiping">\
								<h1>'+m.ip+'</h1><p>'+m.country_name+'</p><p>'+ cityName +'</p>\
								</div>')
						})
						return false;
	...
	closeMiniMap: function () {
		...
		*/
		...
		camera.updateProjectionMatrix();
		$('.mini-map').removeClass('map-act-in').addClass('map-act-out');
	},
